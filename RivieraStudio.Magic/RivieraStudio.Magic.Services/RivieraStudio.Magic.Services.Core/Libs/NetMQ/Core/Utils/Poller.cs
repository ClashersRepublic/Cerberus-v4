...
//        private readonly HashSet<Socket> m_checkWrite = new HashSet<Socket>();
        ...
        {
            this.m_addList.Add(new PollSet(handle, events));

            this.m_checkError.Add(handle);
        ...
        {
            PollSet pollSet = this.m_addList.FirstOrDefault(p => p.Socket == handle);
            ...
            else // this socket is not within any of the PollSets in m_addList.
            {
                ...
                pollSet = this.m_handles.First(p => p.Socket == handle);
                pollSet.Cancelled = true;

                this.m_retired = true;
            }

            this.m_checkError.Remove(handle);
            this.m_checkRead.Remove(handle);
//            m_checkWrite.Remove(handle);
        ...
        */
        ...
        public void Start()
        {
            this.m_workerThread = new Thread(this.Loop) {IsBackground = true, Name = this.m_name};
        ...
        ///     As long as Stop hasn't been called: execute the timers, and invoke the handler-methods on each of the saved
        ///     PollSets.
        /// </summary>
        private void Loop()
        {
...
//            var writeList = new List<Socket>();
            ...
            while (!this.m_stopping)
            {
                ...
                this.m_handles.AddRange(this.m_addList);
                this.m_addList.Clear();

                // Execute any due timers.
                int timeout = this.ExecuteTimers();

                readList.AddRange(this.m_checkRead.ToArray());
//                writeList.AddRange(m_checkWrite.ToArray());
                errorList.AddRange(this.m_checkError.ToArray());
                ...
                foreach (PollSet pollSet in this.m_handles)
                {
...
//                writeList.Clear();
                ...
                if (this.m_retired)
                {
                    ...
                    foreach (PollSet item in this.m_handles.Where(k => k.Cancelled).ToList())
                    {
                        this.m_handles.Remove(item);
                    }

                    this.m_retired = false;
                }
            }
