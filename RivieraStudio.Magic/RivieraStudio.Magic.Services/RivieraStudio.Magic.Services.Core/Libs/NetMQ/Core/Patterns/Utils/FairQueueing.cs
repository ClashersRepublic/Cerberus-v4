...
namespace RivieraStudio.Magic.Services.Core.Libs.NetMQ.Core.Patterns.Utils
{
    ...
    internal class FairQueueing
    {
        ...
        {
            return this.RecvPipe(null, ref msg);
        }
        ...
        public bool RecvPipe(Pipe[] pipe, ref Msg msg)
        {
            ...
            while (this.m_active > 0)
            {
                ...
                bool fetched = this.m_pipes[this.m_current].Read(ref msg);
                ...
                if (fetched)
                {
                    if (pipe != null)
                    {
                        pipe[0] = this.m_pipes[this.m_current];
                    }
        ...
        public bool HasIn()
        {
            ...
            while (this.m_active > 0)
            {
                if (this.m_pipes[this.m_current].CheckRead())
                {
                    return true;
                }
