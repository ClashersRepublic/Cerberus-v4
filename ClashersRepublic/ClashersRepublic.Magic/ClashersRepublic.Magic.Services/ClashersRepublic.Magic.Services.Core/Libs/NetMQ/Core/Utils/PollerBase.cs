...
namespace ClashersRepublic.Magic.Services.Core.Libs.NetMQ.Core.Utils
{
    ...
    internal abstract class PollerBase
    {
        ...
        /// <summary>
        ///     This is a list of key/value pairs, with the keys being timeout numbers and the corresponding values being a list of
        ///     TimerInfo objects.
        ///     It is sorted by the keys - which are timeout values. Thus, by walking down the list, you encounter the soonest
        ///     timeouts first.
        /// </summary>
        ...
        {
            this.m_timers = new SortedList<long, List<TimerInfo>>();
        }
        ...
        {
            Interlocked.Add(ref this.m_load, amount);
        }

        /// <summary>
        ///     Add a <see cref="TimerInfo" /> to the internal list, created from the given sink and id - to expire in the given
        ...
        public void AddTimer(long timeout, [NotNull] IProactorEvents sink, int id)
        {
            ...
            if (!this.m_timers.ContainsKey(expiration))
            {
                this.m_timers.Add(expiration, new List<TimerInfo>());
            }

            this.m_timers[expiration].Add(info);
        }
        ...
        protected int ExecuteTimers()
        {
            ...
            IList<long> keys = this.m_timers.Keys;
            for (int i = 0; i < keys.Count; i++)
            {
                ...
                List<TimerInfo> timers = this.m_timers[key];

                // Trigger the timers.
                foreach (TimerInfo timer in timers)
                {
