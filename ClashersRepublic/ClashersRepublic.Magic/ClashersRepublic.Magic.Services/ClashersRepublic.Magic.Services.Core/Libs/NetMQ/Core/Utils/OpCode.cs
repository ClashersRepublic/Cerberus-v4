...
namespace ClashersRepublic.Magic.Services.Core.Libs.NetMQ.Core.Utils
{
    ...
    internal static class Opcode
    {
        ...
        public static bool Open()
        {
            ...
            byte[] rdtscCode = IntPtr.Size == 4 ? Opcode.RDTSC_32 : Opcode.RDTSC_64;

            Opcode.s_size = (ulong) rdtscCode.Length;

            if (p == 4 || p == 128)
            {
                // Unix
                if (Opcode.IsARMArchitecture())
                {
                    return false;
                }
                ...
                Opcode.s_codeBuffer = (IntPtr) mmap.Invoke(null,
                    new[] {IntPtr.Zero, Opcode.s_size, mmapProtsParam, mmapFlagsParam, -1, 0});

                if (Opcode.s_codeBuffer == IntPtr.Zero || Opcode.s_codeBuffer == (IntPtr) (-1))
                {
            ...
            {
                // Windows
                Opcode.s_codeBuffer = NativeMethods.VirtualAlloc(IntPtr.Zero,
                    (UIntPtr) Opcode.s_size, AllocationType.COMMIT | AllocationType.RESERVE,
                    MemoryProtection.EXECUTE_READWRITE);
            }

            Marshal.Copy(rdtscCode, 0, Opcode.s_codeBuffer, rdtscCode.Length);

            Opcode.Rdtsc = Marshal.GetDelegateForFunctionPointer(Opcode.s_codeBuffer, typeof(RdtscDelegate)) as RdtscDelegate;
        ...
        public static void Close()
        {
            ...
            if (p == 4 || p == 128)
            {
                ...
                munmap.Invoke(null, new object[] {Opcode.s_codeBuffer, Opcode.s_size});
            }
            else
            {
                // Windows
                NativeMethods.VirtualFree(Opcode.s_codeBuffer, UIntPtr.Zero, FreeType.RELEASE);
            }
        }
        ...
        private static class NativeMethods
        {
            ...
            [DllImport(NativeMethods.Kernel, CallingConvention = CallingConvention.Winapi)]
