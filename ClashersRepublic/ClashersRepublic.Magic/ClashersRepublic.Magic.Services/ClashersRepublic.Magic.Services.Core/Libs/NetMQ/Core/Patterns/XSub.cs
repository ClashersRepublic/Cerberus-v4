...
namespace ClashersRepublic.Magic.Services.Core.Libs.NetMQ.Core.Patterns
{
    using System.Diagnostics;
    using ClashersRepublic.Magic.Services.Core.Libs.NetMQ.Core.Patterns.Utils;
    ...
    internal class XSub : SocketBase
    {
        ...
        static XSub()
        {
            XSub.s_sendSubscription = (data, size, arg) =>
            {
                Pipe pipe = (Pipe) arg;
        ...
        {
            this.m_options.SocketType = ZmqSocketType.Xsub;
            this.m_hasMessage = false;
            this.m_moreIn = false;

            this.m_options.Linger = 0;
            this.m_fairQueueing = new FairQueueing();
            this.m_distribution = new Distribution();
            this.m_subscriptions = new Trie();

            this.m_message = new Msg();
            this.m_message.InitEmpty();
        }
        ...
        protected override void XAttachPipe(Pipe pipe, bool icanhasall)
        {
            ...
            this.m_subscriptions.Apply(XSub.s_sendSubscription, pipe);
            pipe.Flush();
        }
        ...
        protected override bool XSend(ref Msg msg)
        {
            ...
            try
            {
                if (!this.m_moreOut && size > 0 && msg[0] == 1)
                {
                    // Process the subscription.
                    if (this.m_subscriptions.Add(msg.Data, msg.Offset + 1, size - 1))
                    {
                        this.m_distribution.SendToAll(ref msg);
                        return true;
                    }
                }
                else if (!this.m_moreOut && size > 0 && msg[0] == 0)
                {
                    if (this.m_subscriptions.Remove(msg.Data, msg.Offset + 1, size - 1))
                    {
                        this.m_distribution.SendToAll(ref msg);
                        return true;
                    }
        ...
        protected override bool XRecv(ref Msg msg)
        {
            ...
            while (true)
            {
                ...
                bool isMessageAvailable = this.m_fairQueueing.Recv(ref msg);
                ...
                if (this.m_moreIn || !this.m_options.Filter || this.Match(msg))
                {
                    this.m_moreIn = msg.HasMore;
                    return true;
                }
                ...
                while (msg.HasMore)
                {
                    isMessageAvailable = this.m_fairQueueing.Recv(ref msg);
        ...
        protected override bool XHasIn()
        {
            ...
            {
                // Get a message using fair queueing algorithm.
                bool isMessageAvailable = this.m_fairQueueing.Recv(ref this.m_message);
                ...
                // Check whether the message matches at least one subscription.
                if (!this.m_options.Filter || this.Match(this.m_message))
                {
                ...
                // Message doesn't match. Pop any remaining parts of the message
                // from the pipe.
                while (this.m_message.HasMore)
                {
                    isMessageAvailable = this.m_fairQueueing.Recv(ref this.m_message);
        ...
        {
            return this.m_subscriptions.Check(msg.Data, msg.Offset, msg.Size);
        }
    }
}