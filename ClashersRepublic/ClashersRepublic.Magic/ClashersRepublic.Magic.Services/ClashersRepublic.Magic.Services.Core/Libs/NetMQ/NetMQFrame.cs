namespace ClashersRepublic.Magic.Services.Core.Libs.NetMQ
{
    ...
    /// <summary>
    ///     Objects of class NetMQFrame serve to hold a Buffer (that consists of a byte-array containing a unit of a
    ...
    public class NetMQFrame : IEquatable<NetMQFrame>, IEquatable<byte[]>
    {
        ...
        {
            return Encoding.ASCII.GetString(this.Buffer, 0, this.MessageSize);
        }
        ...
        {
            return encoding.GetString(this.Buffer, 0, this.MessageSize);
        }
        ...
        {
            if (other?.Length != this.MessageSize)
            {
                return false;
            }

            if (object.ReferenceEquals(this.Buffer, other))
            {
                return true;
            }
        ...
        public bool Equals([CanBeNull] NetMQFrame other)
        {
            ...
            if (this.MessageSize > other.BufferSize || this.MessageSize != other.MessageSize)
            {
                return false;
            }
        ...
        {
            return this.Equals(obj as NetMQFrame);
        }
        ...
        public static bool operator ==(NetMQFrame one, NetMQFrame other)
        {
            ...
            if (object.ReferenceEquals(one, null) && object.ReferenceEquals(other, null))
            {
                return true;
            }

            return !object.ReferenceEquals(one, null) && one.Equals(other);
        }
        ...
        /// <returns>
        ///     the Buffer as a byte-array, either newly-allocated or else (if copy is false) simply a reference to the actual
        ///     Buffer
        /// </returns>
        [NotNull]
        public byte[] ToByteArray(bool copy = false)
        {
            if (!copy || this.MessageSize == this.BufferSize)
            {
                return this.Buffer;
            }

            byte[] byteArray = new byte[this.MessageSize];

            System.Buffer.BlockCopy(this.Buffer, 0, byteArray, 0, this.MessageSize);
