...
namespace LineageSoft.Magic.Services.Core.Libs.NetMQ.Core
{
    ...
    internal class Reaper : ZObject, IPollEvents
    {
        ...
        /// </summary>
        [NotNull] private readonly Poller m_poller;
        ...
        {
            this.m_sockets = 0;
            this.m_terminating = false;

            string name = "reaper-" + threadId;
            this.m_poller = new Poller(name);

            this.m_mailbox = new Mailbox(name);

            this.m_mailboxHandle = this.m_mailbox.Handle;
            this.m_poller.AddHandle(this.m_mailboxHandle, this);
            this.m_poller.SetPollIn(this.m_mailboxHandle);
        }
        ...
        public void InEvent()
        {
            while (true)
            {
                ...
                Command command;
                if (!this.m_mailbox.TryRecv(0, out command))
                {
                    break;
                }
        ...
        protected override void ProcessStop()
        {
            ...
            {
                this.SendDone();
                this.m_poller.RemoveHandle(this.m_mailboxHandle);
                this.m_poller.Stop();
            }
        }
        ...
        protected override void ProcessReap(SocketBase socket)
        {
            ...
            socket.StartReaping(this.m_poller);
        ...
        protected override void ProcessReaped()
        {
            ...
            // finish immediately.
            if (this.m_sockets == 0 && this.m_terminating)
            {
                this.SendDone();
                this.m_poller.RemoveHandle(this.m_mailboxHandle);
                this.m_poller.Stop();
