...
namespace LineageSoft.Magic.Services.Core.Libs.NetMQ.Core
{
    ...
    internal class Pipe : ZObject
    {
        ...
        private enum State
        {
            ...
            /// <summary>
            ///     Pending means that term command was already received from the peer but there are still pending messages to
            ///     read.
            /// </summary>
            Pending,

            /// <summary>
            ///     Terminating means that all pending messages were already read and all we are waiting for is ack from the
            ///     peer.
            /// </summary>
            Terminating,
        ...
        {
            if (!this.m_inActive || this.m_state != State.Active && this.m_state != State.Pending)
            {
                return false;
            }

            // Check if there's an item in the pipe.
            if (!this.m_inboundPipe.CheckRead())
            {
            ...
            if (this.m_inboundPipe.Probe().IsDelimiter)
            {
                Msg msg = new Msg();
                bool ok = this.m_inboundPipe.TryRead(out msg);
                Debug.Assert(ok);
                this.Delimit();
                return false;
            }
        ...
        public bool Read(ref Msg msg)
        {
            ...
            if (!this.m_inboundPipe.TryRead(out msg))
            {
                this.m_inActive = false;
                return false;
            }
            ...
            if (!msg.HasMore)
            {
                this.m_numberOfMessagesRead++;
            }

            if (this.m_lowWatermark > 0 && this.m_numberOfMessagesRead % this.m_lowWatermark == 0)
            {
                this.SendActivateWrite(this.m_peer, this.m_numberOfMessagesRead);
            }
        ...
        {
            if (!this.m_outActive || this.m_state != State.Active)
            {
                return false;
            }

            bool full = this.m_highWatermark > 0 && this.m_numberOfMessagesWritten - this.m_peersMsgsRead == this.m_highWatermark;
        ...
        public bool Write(ref Msg msg)
        {
            ...
            this.m_outboundPipe.Write(ref msg, more);

            if (!more)
            {
                this.m_numberOfMessagesWritten++;
            }
        ...
        public void Rollback()
        {
            ...
            {
                Msg msg = new Msg();
                while (this.m_outboundPipe.Unwrite(ref msg))
                {
        ...
        public void Flush()
        {
            ...
            if (this.m_outboundPipe != null && !this.m_outboundPipe.Flush())
            {
                this.SendActivateRead(this.m_peer);
            }
        }
        ...
        protected override void ProcessActivateWrite(long msgsRead)
        {
            ...
            this.m_outActive = true;
            this.m_sink.WriteActivated(this);
        }
        ...
        protected override void ProcessPipeTerm()
        {
            ...
            if (this.m_state == State.Delimited)
            {
                this.m_state = State.Terminating;
                this.m_outboundPipe = null;
                this.SendPipeTermAck(this.m_peer);
            ...
            // own ack.
            if (this.m_state == State.Terminated)
            {
                this.m_state = State.DoubleTerminated;
                this.m_outboundPipe = null;
                this.SendPipeTermAck(this.m_peer);
                return;
            }
        ...
        protected override void ProcessPipeTermAck()
        {
            ...
            {
                Debug.Assert(this.m_state == State.Terminating || this.m_state == State.DoubleTerminated);
            }
            ...
            // the ypipe itself.
            Msg msg = new Msg();
            while (this.m_inboundPipe.TryRead(out msg))
            {
                msg.Close();
            }
        ...
        {
            return base.ToString() + "[" + this.m_parent + "]";
        }
    }
}