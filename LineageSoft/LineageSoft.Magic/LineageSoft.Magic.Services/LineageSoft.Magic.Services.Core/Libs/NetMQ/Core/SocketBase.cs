...
namespace LineageSoft.Magic.Services.Core.Libs.NetMQ.Core
{
    using System;
    ...
    using AsyncIO;
    using LineageSoft.Magic.Services.Core.Libs.NetMQ.Core.Patterns;
    using LineageSoft.Magic.Services.Core.Libs.NetMQ.Core.Transports.Ipc;
    using LineageSoft.Magic.Services.Core.Libs.NetMQ.Core.Transports.Pgm;
    using LineageSoft.Magic.Services.Core.Libs.NetMQ.Core.Transports.Tcp;
    using LineageSoft.Magic.Services.Core.Libs.NetMQ.Core.Utils;
    using JetBrains.Annotations;
    using TcpListener = LineageSoft.Magic.Services.Core.Libs.NetMQ.Core.Transports.Tcp.TcpListener;
    ...
    internal abstract class SocketBase : Own, IPollEvents, Pipe.IPipeEvents
    {
        ...
        {
            this.m_options.SocketId = socketId;
            this.m_mailbox = new Mailbox("socket-" + socketId);
        }
        ...
        private void CheckContextTerminated()
        {
            if (this.m_isStopped)
            {
                throw new TerminatingException(null, "CheckContextTerminated - yes, is terminated.");
            }
        }
        ...
        private void CheckProtocol([NotNull] string protocol)
        {
            switch (protocol)
            {
                ...
                case Address.EpgmProtocol:
                    ...
                    switch (this.m_options.SocketType)
                    {
                        case ZmqSocketType.Pub:
                        case ZmqSocketType.Sub:
                        case ZmqSocketType.Xpub:
                        ...
                        default:
                            throw new ProtocolNotSupportedException(
                                "Multicast protocols are not supported by socket type: " + this.m_options.SocketType);
                    }
        ...
        {
            this.CheckContextTerminated();

            // First, check whether specific socket type overloads the option.
            if (!this.XSetSocketOption(option, optionValue))
            {
                // If the socket type doesn't support the option, pass it to
                // the generic option parser.
                this.m_options.SetSocketOption(option, optionValue);
            }
        }
        ...
        ///     If the Events option is specified, then process any outstanding commands, and return -1 if that throws a
        ...
        public int GetSocketOption(ZmqSocketOption option)
        {
            ...
            return (int) this.GetSocketOptionX(option);
        }
        ...
        public object GetSocketOptionX(ZmqSocketOption option)
        {
            ...
            if (option == ZmqSocketOption.ReceiveMore)
            {
                return this.m_rcvMore;
            }

            if (option == ZmqSocketOption.Handle)
            {
                return this.m_mailbox.Handle;
            }
            ...
            // If the socket type doesn't support the option, pass it to
            // the generic option parser.
            return this.m_options.GetSocketOption(option);
        }
        ...
        public void Bind([NotNull] string addr)
        {
            ...
            SocketBase.DecodeAddress(addr, out address, out protocol);
            ...
            switch (protocol)
            {
                case Address.InProcProtocol:
                {
                    Ctx.Endpoint endpoint = new Ctx.Endpoint(this, this.m_options);
                    bool addressRegistered = this.RegisterEndpoint(addr, endpoint);
                    ...
                    this.m_options.LastEndpoint = addr;
                    return;
                }
                case Address.PgmProtocol:
                case Address.EpgmProtocol:
            ...
            switch (protocol)
            {
                case Address.TcpProtocol:
                {
                    TcpListener listener = new TcpListener(ioThread, this, this.m_options);

                    try
                    {
                        listener.SetAddress(address);
                        this.m_port = listener.Port;

                        // Recreate the address string (localhost:1234) in case the port was system-assigned
                        addr = $"tcp://{address.Substring(0, address.IndexOf(':'))}:{this.m_port}";
                    }
                    catch (NetMQException ex)
                    {
                        listener.Destroy();
                        this.EventBindFailed(addr, ex.ErrorCode);
                        throw;
                    }

                    this.m_options.LastEndpoint = listener.Address;
                ...
                case Address.EpgmProtocol:
                {
                    PgmListener listener = new PgmListener(ioThread, this, this.m_options);
                ...
                case Address.IpcProtocol:
                {
                    IpcListener listener = new IpcListener(ioThread, this, this.m_options);
        ...
        public void Connect([NotNull] string addr)
        {
            ...
            if (protocol == Address.InProcProtocol)
            {
                ...
                int sndhwm = this.m_options.SendHighWatermark != 0 && peer.Options.ReceiveHighWatermark != 0
                    ? this.m_options.SendHighWatermark + peer.Options.ReceiveHighWatermark
                    : 0;

                int rcvhwm = this.m_options.ReceiveHighWatermark != 0 && peer.Options.SendHighWatermark != 0
                    ? this.m_options.ReceiveHighWatermark + peer.Options.SendHighWatermark
                    : 0;

                // The total LWM for an inproc connection should be the sum of
                // the binder's LWM and the connector's LWM.
                int sndlwm = this.m_options.SendLowWatermark != 0 && peer.Options.ReceiveLowWatermark != 0
                    ? this.m_options.SendLowWatermark + peer.Options.ReceiveLowWatermark
                    : 0;

                int rcvlwm = this.m_options.ReceiveLowWatermark != 0 && peer.Options.SendLowWatermark != 0
                    ? this.m_options.ReceiveLowWatermark + peer.Options.SendLowWatermark
                    : 0;
                ...
                bool[] delays = {this.m_options.DelayOnDisconnect, this.m_options.DelayOnClose};
                ...
                {
                    Msg id = new Msg();
                    id.InitPool(this.m_options.IdentitySize);
                    id.Put(this.m_options.Identity, 0, this.m_options.IdentitySize);
                ...
                this.m_inprocs.Add(addr, pipes[0]);
            ...
            switch (protocol)
            {
                case Address.TcpProtocol:
                {
                    ...
                    paddr.Resolved.Resolve(address, this.m_options.IPv4Only);
                    break;
                }
                case Address.IpcProtocol:
                {
                ...
                case Address.EpgmProtocol:
                {
                    if (this.m_options.SocketType == ZmqSocketType.Sub || this.m_options.SocketType == ZmqSocketType.Xsub)
                    {
                        this.Bind(addr);
                        return;
                    }
            ...
            // Create session.
            SessionBase session = SessionBase.Create(ioThread, true, this, this.m_options, paddr);
            ...
            if (!this.m_options.DelayAttachOnConnect || icanhasall)
            {
                // Create a bi-directional pipe.
                ZObject[] parents = {this, session};
                int[] hwms = {this.m_options.SendHighWatermark, this.m_options.ReceiveHighWatermark};
                int[] lwms = {this.m_options.SendLowWatermark, this.m_options.ReceiveLowWatermark};
                bool[] delays = {this.m_options.DelayOnDisconnect, this.m_options.DelayOnClose};
            ...
            // Save last endpoint URI
            this.m_options.LastEndpoint = paddr.ToString();
        ...
        /// <summary>
        ///     Take ownership of the given <paramref name="endpoint" /> and register it against the given
        ...
        private void AddEndpoint([NotNull] string address, [NotNull] Own endpoint, Pipe pipe)
        {
            ...
            this.m_endpoints[address] = new Endpoint(endpoint, pipe);
        }
        ...
        public void TermEndpoint([NotNull] string addr)
        {
            ...
            if (protocol == Address.InProcProtocol)
            {
                ...
                Pipe pipe;
                if (!this.m_inprocs.TryGetValue(addr, out pipe))
                {
                    throw new EndpointNotFoundException("Endpoint was not found and cannot be disconnected");
                }

                pipe.Terminate(true);
                this.m_inprocs.Remove(addr);
            }
            else
            {
                Endpoint endpoint;
                if (!this.m_endpoints.TryGetValue(addr, out endpoint))
                {
                    throw new EndpointNotFoundException("Endpoint was not found and cannot be disconnected");
                }
                ...
                this.TermChild(endpoint.Own);
                this.m_endpoints.Remove(addr);
            }
        }
        ...
        /// <param name="timeout">
        ///     The timeout to wait before returning <c>false</c>. Pass
        ///     <see cref="SendReceiveConstants.InfiniteTimeout" /> to disable timeout.
        /// </param>
        ...
        public bool TrySend(ref Msg msg, TimeSpan timeout, bool more)
        {
            ...
            bool isMessageSent = this.XSend(ref msg);
            ...
            while (true)
            {
                this.ProcessCommands(timeoutMillis, false);

                isMessageSent = this.XSend(ref msg);
        ...
        /// <summary>
        ///     Receive a frame into the given <paramref name="msg" /> and return <c>true</c> if successful, <c>false</c> if it
        ///     timed out.
        /// </summary>
        ...
        ///         <item>
        ///             <see cref="SendReceiveConstants.InfiniteTimeout" /> (or a negative value) - wait indefinitely, always
        ///             returning <c>true</c>
        ///         </item>
        ///         <item>
        ///             Any positive value - return <c>false</c> after the corresponding duration if no message has become
        ///             available
        ///         </item>
        ///     </list>
        /// </remarks>
        ...
        public bool TryRecv(ref Msg msg, TimeSpan timeout)
        {
            ...
            // Get the message.
            bool isMessageAvailable = this.XRecv(ref msg);
            ...
            if (++this.m_ticks == Config.InboundPollRate)
            {
                this.ProcessCommands(0, false);
                this.m_ticks = 0;
            }
            ...
            if (timeout == TimeSpan.Zero)
            {
                ...
                isMessageAvailable = this.XRecv(ref msg);
            ...
            bool block = this.m_ticks != 0;
            while (true)
            {
                this.ProcessCommands(block ? timeoutMillis : 0, false);
        ...
        internal void StartReaping([NotNull] Poller poller)
        {
            ...
            this.m_handle = this.m_mailbox.Handle;
            this.m_poller.AddHandle(this.m_handle, this);
            this.m_poller.SetPollIn(this.m_handle);
        ...
        private void ProcessCommands(int timeout, bool throttle)
        {
            ...
            if (timeout != 0)
            {
                ...
                found = this.m_mailbox.TryRecv(timeout, out command);
            }
            else
            {
                ...
                if (tsc != 0 && throttle)
                {
                    ...
                    if (tsc >= this.m_lastTsc && tsc - this.m_lastTsc <= Config.MaxCommandDelay)
                    {
                        return;
                    }
                ...
                found = this.m_mailbox.TryRecv(0, out command);
            }
        ...
        protected override void ProcessTerm(int linger)
        {
            ...
            {
                this.m_pipes[i].Terminate(false);
            }

            this.RegisterTermAcks(this.m_pipes.Count);
        ...
        private void CheckDestroy()
        {
            ...
            if (this.m_destroyed)
            {
                ...
                this.m_poller.RemoveHandle(this.m_handle);
        ...
        {
            if (this.m_options.DelayAttachOnConnect)
            {
                pipe.Terminate(false);
            }
            else
        ...
        public void Terminated(Pipe pipe)
        {
            ...
            this.XTerminated(pipe);

            // Remove pipe from inproc pipes
            string[] pipesToDelete = this.m_inprocs.Where(i => i.Value == pipe).Select(i => i.Key).ToArray();
        ...
        private void ExtractFlags(ref Msg msg)
        {
            ...
            Debug.Assert(!msg.IsIdentity || this.m_options.RecvIdentity);

            // Remove MORE flag.
            this.m_rcvMore = msg.HasMore;
        }
        ...
        public void Monitor([CanBeNull] string addr, SocketEvents events)
        {
            ...
            this.m_monitorEvents = events;

            this.m_monitorSocket = this.Ctx.CreateSocket(ZmqSocketType.Pair);
            ...
            try
            {
                this.m_monitorSocket.SetSocketOption(ZmqSocketOption.Linger, linger);
            }
            ...
            try
            {
                this.m_monitorSocket.Bind(addr);
            }
            catch (NetMQException)
            {
                this.StopMonitor();
        ...
        {
            if ((this.m_monitorEvents & SocketEvents.Connected) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.Connected, addr, ch));
        }

        public void EventConnectDelayed([NotNull] string addr, ErrorCode errno)
        {
            if ((this.m_monitorEvents & SocketEvents.ConnectDelayed) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.ConnectDelayed, addr, errno));
        }

        public void EventConnectRetried([NotNull] string addr, int interval)
        {
            if ((this.m_monitorEvents & SocketEvents.ConnectRetried) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.ConnectRetried, addr, interval));
        }

        public void EventListening([NotNull] string addr, [NotNull] AsyncSocket ch)
        {
            if ((this.m_monitorEvents & SocketEvents.Listening) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.Listening, addr, ch));
        }

        public void EventBindFailed([NotNull] string addr, ErrorCode errno)
        {
            if ((this.m_monitorEvents & SocketEvents.BindFailed) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.BindFailed, addr, errno));
        }

        public void EventAccepted([NotNull] string addr, [NotNull] AsyncSocket ch)
        {
            if ((this.m_monitorEvents & SocketEvents.Accepted) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.Accepted, addr, ch));
        }

        public void EventAcceptFailed([NotNull] string addr, ErrorCode errno)
        {
            if ((this.m_monitorEvents & SocketEvents.AcceptFailed) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.AcceptFailed, addr, errno));
        }

        public void EventClosed([NotNull] string addr, [NotNull] AsyncSocket ch)
        {
            if ((this.m_monitorEvents & SocketEvents.Closed) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.Closed, addr, ch));
        }

        public void EventCloseFailed([NotNull] string addr, ErrorCode errno)
        {
            if ((this.m_monitorEvents & SocketEvents.CloseFailed) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.CloseFailed, addr, errno));
        }

        public void EventDisconnected([NotNull] string addr, [NotNull] AsyncSocket ch)
        {
            if ((this.m_monitorEvents & SocketEvents.Disconnected) == 0)
            {
                return;
            }

            this.MonitorEvent(new MonitorEvent(SocketEvents.Disconnected, addr, ch));
        }

        private void MonitorEvent([NotNull] MonitorEvent monitorEvent)
        {
            if (this.m_monitorSocket == null)
            {
                return;
            }

            monitorEvent.Write(this.m_monitorSocket);
        }
        ...
        private void StopMonitor()
        {
            if (this.m_monitorSocket != null)
            {
                this.m_monitorSocket.Close();
                this.m_monitorSocket = null;
                this.m_monitorEvents = 0;
            }
        }
